...
> **Estado:** EM CURSO — **7/11 stories Done** (Stories 3.1, 3.2, 3.3, 3.4 e 3.5 fechadas 21-22/05/2026; Story 3.6 fechada 23/05/2026; Story 3.7 fechada 24/05/2026)
...
**Epic 3 EM CURSO — 7/11 stories Done.** As Stories 3.1 (Schema finanças), 3.2 (Categorias default PT), 3.3 (CRUD transações variáveis), 3.4 (CRUD recorrências), 3.5 (CRUD cartões + contas) e 3.6 (Compras parceladas) estão fechadas (`@po *close-story` — 3.1/3.2 em 21/05/2026, 3.3/3.4/3.5 em 22/05/2026, 3.6 em 23/05/2026 — PR #30 squash `06e3cfb6`, PR #31 squash `25fce8a8`, PR #32 squash `1a48855a`, PR #33 squash `54d7f851`, PR #34 squash `51074f28`, PR #35 squash `7be125f4` merged em `main`). As restantes 5 stories (3.7 a 3.11) estão decompostas a partir do PRD §10 Epic 3, cobrindo integralmente os 8 FRs de Finanças (FR16-FR23) e os 5 Epic ACs.
...
**Próximo passo recomendado:** `@sm *draft 3.8` (Vista cartões — fatura corrente + próxima fatura agrupando prestações por mês — FR18+FR19, desbloqueada pela 3.6) **ou** `@sm *draft 3.9` (Vista património — saldo agregado por banco/conta com drilldown — FR20) **ou** `@sm *draft 3.10` (geração diária recorrentes + prestações — FR17+FR19, desbloqueada por 3.4 + 3.6) **ou** `@sm *draft 3.11` (Tools cérebro finanças — FR23, recomendada por último por integrar tudo). As 4 stories são paralelizáveis entre si conforme §10. Eurico decide a sequência. Depois: `@po *validate-story-draft` → executor → quality gate → `@devops *push`. Cada story em feature branch dedicada (`feature/3.X-...`), não em `main`. Hard-stop §8 (máx 2 iter CR) — Epic 3 abriu 1 excepção autorizada na Story 3.7 Iter 3 (refactor DST-safe + handoff render fix, autorização Eurico via trailer `Authorized-by:`). Waiver rate 0/7 mantido.
