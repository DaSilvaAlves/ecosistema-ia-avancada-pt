...
> **Estado:** ✅ **COMPLETO — 11/11 stories Done** (Stories 3.1, 3.2, 3.3, 3.4 e 3.5 fechadas 21-22/05/2026; Story 3.6 fechada 23/05/2026; Story 3.7 fechada 24/05/2026; Story 3.10 fechada 25/05/2026; Stories 3.8 e 3.9 fechadas 28/05/2026; **Story 3.11 fechada 28/05/2026 — Epic 3 COMPLETO**)
...
| 3.6 | Compras parceladas | Compras parceladas vinculadas a cartão — geração de N transações futuras automaticamente | FR19 | `@dev` | `@architect` | **Done** |
| 3.7 | Vista "este mês" | Vista mensal: análise por categoria, por dia, total entrado vs saído, projecção 30 dias incluindo recorrentes e prestações | FR21 | `@ux-design-expert` | `@dev` | **Done** |
| 3.8 | Vista cartões | Vista de cartões: fatura corrente + próxima fatura + prestações | FR18, FR19 | `@ux-design-expert` | `@dev` | **Done** |
| 3.9 | Vista património | Vista Património: saldo agregado por conta/banco com drilldown | FR20 | `@ux-design-expert` | `@dev` | **Done** |
| 3.10 | Geração diária recorrentes + prestações | Motor client-side de geração diária de transações recorrentes + prestações — ao primeiro carregamento do dia (reutiliza `runRecurrenceEngine` da Story 2.7) | FR17, FR19 | `@dev` | `@architect` | **Done** |
| 3.11 | Tools cérebro finanças | Registar 6 tools no Tool Registry (nomes ASCII): `criar_financa_variavel`, `criar_financa_recorrente`, `criar_cartao`, `criar_parcelada`, `consultar_balanco`, `consultar_categoria` | FR23 | `@dev` | `@architect` | **Done** |
...
**✅ Epic 3 COMPLETO — 11/11 stories Done.** As 11 stories estão fechadas (`@po *close-story`): 3.1/3.2 em 21/05/2026, 3.3/3.4/3.5 em 22/05/2026, 3.6 em 23/05/2026, 3.7 em 24/05/2026, 3.10 em 25/05/2026, 3.8/3.9 em 28/05/2026, **3.11 em 28/05/2026 (closer)** — PRs #30 (`06e3cfb6`), #31 (`25fce8a8`), #32 (`1a48855a`), #33 (`54d7f851`), #34 (`51074f28`), #35 (`7be125f4`), #36 (`deac687b`), #37 (`54454a71`), #38 (`b30e781a`), #39 (`adf62343`), **#40 (`e77b0fea`)** merged em `main`. Os 8 FRs de Finanças (FR16-FR23) e os 5 Epic ACs estão integralmente cobertos. A Story 3.11 (FR23) registou as 6 tools de finanças no Tool Registry — o cérebro multi-intent executa agora acções reais de finanças a partir de linguagem natural (AC4 canónico).

**Waiver rate Epic 3: 1/11** (só Story 3.10 — merge waived 2×; alvo <2/11 ATINGIDO). Story 3.11 mergeada CLEAN na Iter 2 (não-waived). Hard-stop §8 respeitado em todas as stories.

**Próximo passo:** `@po *retrospective epic-3` — retrospectiva do Epic 3 (lições, débitos §8 acumulados, métricas de waiver/iteração, acções para o Epic 4).
...
**Story 3.6 — fecho confirmado.** Camada de UI + lógica de domínio de FR19 entregue: helper puro `lib/financas/installmentSplit.ts` (`splitInstallmentAmount` — divisão inteira com base + remainder, invariante de soma exacta validada para casos divisível €1.200/12 e não-divisível €100/3; `installmentDates` — stepping mensal explícito com clamp ao último dia do mês quando o dia-de-origem não existe no destino, justificação para não usar `rrule` MONTHLY+`bymonthday`; coverage 100% lines/branches/functions/statements), modal `InstallmentFormModal` (6 campos: cartão + descrição + categoria + valor total + nº prestações + data início; preview em vivo "N× de €X,XX" com indicação da 1ª parcela se não-divisível; WAI-ARIA dialog completo; create-only — sem edição [AUTO-DECISION] A6), lista `InstallmentsList` (descrição + cartão via lookup + "N× de €X" + total + data PT-PT; só acção Apagar; render-safe try/catch isola row malformada da tab inteira — Fix #2 CR Iter 1), hook `useInstallments` (`useLiveQuery`), repo estendido com `listInstallments` + **atomicidade no repo** (`createInstallmentWithTransactions` + `deleteInstallmentCascade` em `db.transaction('rw', ...)` — mantém regra de repo isolation da page, opção (a) do Dev Notes; `cardId` guard para defesa em profundidade — Fix #1 CR Iter 1); tab strip `/financas` estendido a 5 separadores (Transações, Recorrências, Contas, Cartões, Parceladas); botão "+ Nova parcelada" desactivado com hint "Cria um cartão primeiro" quando `cards.length === 0` (AC7). 4 ficheiros novos + 2 modificados + 1 teste; 16 testes Vitest novos; suite **853/853 PASS**. Architect Gate PASS first-iter confiança ALTA — revisão manual dos cálculos (§9) cruzou 6 inputs incluindo edge `(5, 10)` total<n. [GAP-3.6-1] (categoria form-only — `InstallmentSubmit = { installment, category }`, categoria estampada nas N transações mas não persistida no `Installment` — geração eager + zero edição = sem regeneração = categoria não se perde; schema extension `version(5)` desnecessária e cara) **CONFIRMADO** pelo `@architect`. Boundary com Story 3.10 (A2 — eager generation; 3.10 não precisa de gerar prestações, no máximo reconciliar) **CONFIRMADA** com zero duplicação de lógica entre domínios parceladas/recorrências. CR Iter 1 resolvido com 2 fixes legítimos (Fix #1 cardId guard + Fix #2 render-safe split); Fix #3 (outside-diff "add unit tests for handlers in page.tsx") defendido como **policy do Epic 3** (componentes React validados via `@architect` + CR server-side, sem unit tests próprios) com precedente Stories 3.3/3.4/3.5; CR Iter 2 APPROVED. Hard-stop §8 não atingido. 2 observações LOW informacionais (sentinela `cardId=''` no FormState herdada da Story 3.5, full-table-scan em `deleteInstallmentCascade` por `installmentId` não-indexado em `version(3)`) — **sem novos débitos** registados em §8. Padrão "atomicidade no repo" da Story 3.4 (`deleteFinanceRecurrence`) replicado e estendido.

**Story 3.7 — fecho confirmado.** Vista "Este mês" (FR21) entregue — PR #36 squash-merged em `main` (closure commit `783faca2`). Hard-stop §8 abriu 1 excepção autorizada na Iter 3 (refactor DST-safe + handoff render fix, autorização Eurico via trailer `Authorized-by:`).

**Story 3.10 — fecho confirmado.** Controlo de activação dos motores diários (FR17 + FR19) entregue: helper puro `lib/financas/dailyRunGate.ts` (`getTodayLocalIso(now)` + `shouldRunDailyEngine(todayIso, lastRunIso)` — sem React, sem `localStorage`, coverage 100%), hook `useDailyGenerationEngine` (gate `localStorage[nexus:lastDailyEngineRun]` + sequência `await` `runRecurrenceEngine` → `runFinanceRecurrenceEngine` — só persiste novo `lastRun` se ambos os motores completarem sem erro), provider client trivial `components/system/DailyEngineProvider.tsx` envolvendo `{children}` em `app/(app)/layout.tsx`, remoção de `useRecurrenceEngine`/`useFinanceRecurrenceEngine` das 3 pages individuais (`tarefas/page.tsx` + `financas/page.tsx` + `financas/mes/page.tsx`) + `@deprecated` JSDoc nos hooks individuais (preservados funcionais para evitar quebras de imports). 23 testes novos (15 helper + 8 hook); suite **913/913 PASS** locais; build PASS; E2E PASS; 50-prompt regression PASS; CodeQL PASS; coverage PASS; Vercel deployed. Boundary 3.6 preservada (zero código de parceladas na 3.10 — [AUTO-DECISION] A5). ADR-2.7-1 preservado (gate vive dentro do `useEffect` como early-return, sem ServiceWorker e sem `setInterval`). Pax PO Validation GO 9.5/10. PR #37 squash-merged em `main` (squash commit `54454a71`). **3 iterações de CodeRabbit (zero CRITICAL/HIGH em todas as 3 iter)**; merge fechado com **2× `AUTHORIZATION-§8` documentadas no commit body** (decisão consciente do Eurico — hard-stop §8 e merge waived autorizados explicitamente). **Waiver rate Epic 3: 1/8** (primeira waiver do epic; as 7 stories anteriores fecharam dentro do hard-stop sem waiver).

**Story 3.8 — fecho confirmado.** Vista de cartões (FR18+FR19) entregue — PR #38 squash-merged em `main` (commit `b30e781a`, 28/05/2026 07:24 UTC). CR Iter 2 fechado via reply ao falso positivo (`cardBilling.ts:155` — guard A2 já implementado em `cardBilling.ts:146-151`) + doc-nits no story file — NÃO Iter 3, sem waiver. Quality gates locais 4/4 PASS (941/941 testes). `@po *close-story 3.8` 28/05/2026.

**Story 3.9 — fecho confirmado.** Vista Património (FR20) entregue — PR #39 squash-merged em `main` (commit `adf62343`, 28/05/2026 17:01 UTC). 3 iterações CR: Iter 1 (Major — sem unit tests para `PatrimonioPage`), Iter 2 (tests C1-C5 criados, mas CR pediu cobertura do 3º tipo de conta `cash`), Iter 3 autorizada por Eurico (hard-stop §8 — finding trivial só no ficheiro de teste: cobertura `cash` no C3 + IDs determinísticos). CR Iter 3 verde (0 actionable comments), todos os checks SUCCESS, merge sem waiver (reviewDecision stale confirmado — CR não submete review formal APPROVED quando 0 actionable, só comment + check `success`; padrão Story 1.10). Quality gates locais 3/3 PASS (962/962 testes). `@po *close-story 3.9` 28/05/2026.

**Story 3.11 — fecho confirmado (closer do Epic 3).** Tools cérebro finanças (FR23) entregue — PR #40 squash-merged em `main` (commit `e77b0fea`, 28/05/2026). `lib/agent/tools/finance.ts` regista 6 tools no domínio `finance` (nomes ASCII por imposição do `TOOL_NAME_PATTERN` + Anthropic spec — DEV-DECISION D-NAMES ratificada pela Aria): `criar_financa_variavel`, `criar_financa_recorrente`, `criar_cartao`, `criar_parcelada`, `consultar_balanco`, `consultar_categoria`. `toolRegistry.all()` = 13 (7 Epic 2 + 6 Epic 3). Cadeia completa: fix-story (NO-GO→GO 9,5/10) → develop → Architect Gate PASS → push → CR Iter 1 (6 actionable: 2 Major código `validateContaCartao`+`isoDateField`, 1 Major doc cedilha→ASCII, 3 Minor) → fixes Iter 1 → **CR Iter 2 limpo (0 issues novos) → merge CLEAN (não-waived)**. Schema 100% alinhado com `types/db.ts` (cross-link de recorrência valida com `runFinanceRecurrenceEngine`). 988/988 testes, build edge PASS, coverage `finance.ts` 98,68%. AC12 (verificação manual V1-V4) fica para validação pós-deploy em staging. `@po *close-story 3.11` 28/05/2026.

**✅ EPIC 3 COMPLETO (11/11).** **Waiver rate final: 1/11** (só Story 3.10 — alvo <2/11 ATINGIDO). Próximo: `@po *retrospective epic-3`.
