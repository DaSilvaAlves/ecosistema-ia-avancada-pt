...
> **Estado:** EM CURSO — **3/11 stories Done** (Stories 3.1, 3.2 e 3.3 fechadas 21-22/05/2026)
...
**Epic 3 EM CURSO — 3/11 stories Done.** As Stories 3.1 (Schema finanças), 3.2 (Categorias default PT) e 3.3 (CRUD transações variáveis) estão fechadas (`@po *close-story` — 3.1/3.2 em 21/05/2026, 3.3 em 22/05/2026 — PR #30 squash `06e3cfb6`, PR #31 squash `25fce8a8`, PR #32 squash `1a48855a` merged em `main`). As restantes 8 stories (3.4 a 3.11) estão decompostas a partir do PRD §10 Epic 3, cobrindo integralmente os 8 FRs de Finanças (FR16-FR23) e os 5 Epic ACs.
...
**Próximo passo recomendado:** `@sm *draft 3.4` (CRUD recorrências — FR17, reutiliza o motor `runRecurrenceEngine` da Story 2.7). A 3.4 está desbloqueada pela 3.1. Depois: `@po *validate-story-draft 3.4` → executor `@dev *develop 3.4` → quality gate `@qa` → `@devops *push`. A Story 3.4 nasce em feature branch dedicada (`feature/3.4-...`), não em `main`.
