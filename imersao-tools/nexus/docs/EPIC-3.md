...
> **Estado:** EM CURSO — **6/11 stories Done** (Stories 3.1, 3.2, 3.3, 3.4 e 3.5 fechadas 21-22/05/2026; Story 3.6 fechada 23/05/2026)
...
**Próximo passo recomendado:** `@sm *draft 3.7` (Vista "este mês" — análise por categoria/dia, total entrado vs saído, projecção 30 dias — FR21) **ou** `@sm *draft 3.8` (Vista cartões — fatura corrente + próxima fatura agrupando prestações por mês — FR18+FR19, agora desbloqueada pela 3.6) **ou** `@sm *draft 3.10` (geração diária recorrentes + prestações — FR17+FR19, desbloqueada por 3.4 + 3.6). As três stories são paralelizáveis entre si conforme §10 (`@sm`/`@po` confirmam por story). Eurico decide a sequência. Depois: `@po *validate-story-draft` → executor → quality gate → `@devops *push`. Cada story em feature branch dedicada (`feature/3.X-...`), não em `main`. Hard-stop §8 (máx 2 iter CR) e waiver rate 0/6 mantém-se na expectativa.
