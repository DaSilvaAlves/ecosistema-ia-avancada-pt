...
> **Estado:** EM CURSO — **4/11 stories Done** (Stories 3.1, 3.2, 3.3 e 3.4 fechadas 21-22/05/2026)
...
**Epic 3 EM CURSO — 4/11 stories Done.** As Stories 3.1 (Schema finanças), 3.2 (Categorias default PT), 3.3 (CRUD transações variáveis) e 3.4 (CRUD recorrências) estão fechadas (`@po *close-story` — 3.1/3.2 em 21/05/2026, 3.3/3.4 em 22/05/2026 — PR #30 squash `06e3cfb6`, PR #31 squash `25fce8a8`, PR #32 squash `1a48855a`, PR #33 squash `54d7f851` merged em `main`). As restantes 7 stories (3.5 a 3.11) estão decompostas a partir do PRD §10 Epic 3, cobrindo integralmente os 8 FRs de Finanças (FR16-FR23) e os 5 Epic ACs.
...
**Próximo passo recomendado:** `@sm *draft 3.5` (CRUD cartões de crédito com fecho de fatura + dia de vencimento; contas bancárias com saldo — FR18). A 3.5 está desbloqueada pela 3.1. Depois: `@po *validate-story-draft 3.5` → executor `@dev *develop 3.5` → quality gate `@qa` → `@devops *push`. A Story 3.5 nasce em feature branch dedicada (`feature/3.5-...`), não em `main`. Nota de sequenciamento: a Story 3.6 (compras parceladas) depende da 3.5 — as parceladas vinculam-se a cartão.
