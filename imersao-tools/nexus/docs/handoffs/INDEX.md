...
|---------|-----|------|------|--------------|
| [RETOMA-20260511-story-1.10-iter3-fix-pronto-para-devops-push.md](RETOMA-20260511-story-1.10-iter3-fix-pronto-para-devops-push.md) | `@dev` (Dex) | `@devops` (Gage) | 11/05/2026 | **HANDOFF @dev → @devops** — Iter 3 fix do mock SSE protocol aplicado. Causa raiz REAL: `mock-events.ts` divergia do `executor.ts` em 5 pontos (faltava `phase: 'start'` em meta, `text` vs `delta`, `done` sem intents/tokens/totals, formato wire `event:` vs `data:`-only, sem `[DONE]`). Resultado: useAgentStream não criava runId → MessageList retornava `null` → ToolCards/text nunca renderizavam → 30s timeout + browser closed. **Validação local: 48/50 PASS, threshold met (≥43), Canonical PASS, P95 252ms**. Commit local `21f91867`. Comando: **`@devops *push`**. |
...
|---------|-----|------|----------|-----|
| [RETOMA-20260510-story-1.10-pr-14-iter3-ci-vermelho-browser-crash-CONSUMIDO.md](archive/RETOMA-20260510-story-1.10-pr-14-iter3-ci-vermelho-browser-crash-CONSUMIDO.md) | `@devops` (Gage) | `@dev` (Dex) | 11/05/2026 | Dex (@dev) — diagnóstico Iter 3 reproduzido localmente; causa raiz real identificada (mock SSE protocol divergente); fix aplicado em 4 ficheiros (mock-events, route-handler, stream-wait, regression spec); 48/50 PASS local; commit `21f91867` |
