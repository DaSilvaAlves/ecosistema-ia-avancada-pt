...
> **Estado:** EM CURSO — 9/10 stories Done (4.1 schema/DAL, 4.2 CRUD hábitos, 4.3 heatmap calendário, 4.4 métricas por hábito, 4.5 CRUD metas, 4.6 CRUD lembretes, 4.7 setup Web Push, 4.8 disparo server-side de push agendado, 4.10 tools cérebro). Resta só 1 de Web Push: 4.9 (SW handler — display visível + botões), que consome o disparo agendado entregue pela 4.8. A 4.8 fechou em 03/06 (PR #55 `6b429560`; produção `017a032c` pós hotfix middleware PR #56; scheduler cron-job.org LIVE).
...
**Epic 4 EM CURSO — 8/10 stories Done (4.1, 4.2, 4.3, 4.4, 4.5, 4.6, 4.7, 4.10).** As Stories 4.1 (schema/DAL, PR #41 merge `aa74ce56`), 4.2 (CRUD hábitos, PR #42), 4.3 (heatmap calendário), 4.4 (métricas por hábito, PR #50 merge `192b488c`), 4.5 (CRUD metas, PR #52 merge `a7078291`), 4.6 (CRUD lembretes, PR #51 merge `d13a6067`), 4.7 (setup Web Push, PR #54 merge `25d1c780`) e 4.10 (tools cérebro, PR #53 merge `09d52b24`) estão DONE em `main`. Restam 2 stories — ambas de Web Push: 4.8 (agendamento push) e 4.9 (SW push handler), ambas destrancadas pela infra da 4.7. **Próxima: 4.8** (agendamento push — depende de 4.6 lembretes + 4.7 infra, ambas DONE) ou **4.9** (SW push handler — depende de 4.7, DONE; entrega o display visível da notificação + botões accionáveis). As decisões arquitecturais da 4.1 estão fixadas: `goal_milestones` EMBEBIDO (sem `version(5)`); convenção de delete-cascata = *cascade nos filhos sem vida própria (composição); set null nas referências a entidades com vida própria (associação)*; hard-delete; corolário D6 (`Task.projectId` → set null) ratificado como referência para a story técnica pós-Epic-4.
