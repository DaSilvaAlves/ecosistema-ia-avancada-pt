...
> **Estado:** EM CURSO — 2/10 stories Done (4.1 schema/DAL, 4.2 CRUD hábitos). Próxima: 4.3 (heatmap calendário) / 4.5 (CRUD metas) / 4.6 (CRUD lembretes) — todas desbloqueadas.
...
| 4.3 | Heatmap calendário | Heatmap estilo GitHub contributions por hábito, últimos 6 meses. **Componente com múltiplos estados de render → teste de componente obrigatório (`react-component-test-criteria.md`)** | FR26 | `@ux-design-expert` | `@dev` | **Ready for Review** (29/05 — `docs/stories/active/4.3.story.md`; PO Validation GO 10/10 `PO-VALIDATION-STORY-4.3.md`; implementada por Uma + quality gate `@dev` PASS; PR #43 aberto, CodeRabbit Iter 1→2 em curso, aguarda merge) |
