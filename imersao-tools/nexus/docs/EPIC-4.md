...
> **Estado:** FECHADO — 10/10 stories Done (4.1 schema/DAL, 4.2 CRUD hábitos, 4.3 heatmap calendário, 4.4 métricas por hábito, 4.5 CRUD metas, 4.6 CRUD lembretes, 4.7 setup Web Push, 4.8 disparo server-side de push agendado, 4.9 SW push handler, 4.10 tools cérebro). A 4.9 (SW handler — display visível + botões "marcar feito"/"snooze") fechou em 06/06 (PR #58 squash-merge `64a41445`), consumindo o disparo agendado da 4.8 (PR #55 `6b429560`; produção `017a032c` pós hotfix middleware PR #56; scheduler cron-job.org LIVE). **AC13 da 4.9** (verificação manual display+botões Chrome+Edge) fica para verificação em produção pós-merge, autorizada pelo Eurico (preview do PR sem env vars VAPID; produção tem). Próximo: Eurico AC13 prod + `@po *retrospective epic-4`.
...
| 4.5 | CRUD metas + vista | CRUD metas (título, prazo, métrica numérica/booleana, target, actual, milestones) + vista com progress bar + histórico + milestones | FR39, FR40 | `@dev` | `@qa` | **DONE** — PR #52 merged `a7078291` (01/06). Executor foi `@dev` (não `@ux-design-expert` planeado) → gate subiu a `@qa` por `separation-of-roles.md`. Helper puro `lib/metas/progress.ts` (100% cov) + `GoalProgressBar`/`GoalFormModal`/`GoalsList`/`GoalView` + página `/metas` (TabStrip 3 tabs) + NavLink. `Goal.progressLog?` embebido não-indexado (sem version bump, precedente `Habit.archivedAt?`). QA gate PASS 12/12 AC, CI 100% verde, CR 2 iter 0 CRITICAL (F1/F2/F3 resolvidos), Vitest 1246/1246, zero waivers. Desbloqueia a 4.10 |
...
| 4.7 | Setup Web Push | VAPID keys (self-generated), fluxo de subscrição no browser (FR35), endpoint `/api/push/send`. **GAP arquitectural: Edge vs Node runtime; secrets VAPID — ver §7** | FR34, FR35 | `@dev` | `@architect` | **DONE** — PR #54 squash-merged `25d1c780` (02/06). Infra Web Push: VAPID Prod+Dev provisionados (@devops), Vercel KV vivo, SW mínimo `/sw.js` (stub push p/ 4.9), endpoints `/api/push/subscribe` + `/api/push/send` (Node runtime), helper `subscriptions-store.ts` (KV server-only) + `utils.ts`, hook `usePushSubscription`, componente `PushPermissionPrompt` montado em `/lembretes`. Architect Gate Aria CONCERNS (FR35 onboarding = follow-up; CR pre-PR cumprido). Smoke AC14 Chrome+Edge real (FCM aceitou entrega 200). CR Iter 2 APPROVED, CI rollup CLEAN, Vitest 1292/1292, zero waivers |
| 4.8 | Disparo server-side de push agendado | **Opção A′ (Architect Gate de entrada Aria):** disparo server-side, não client on-mount — `sendPushNotification` extraída de `/api/push/send` + mirror de agenda em KV (`/api/push/schedule`, hash `nexus:push:schedule`) + `/api/push/dispatch` protegido por `CRON_SECRET` (Bearer) chamado por scheduler ao minuto + reconciliação `sent`→Dexie on-mount. Recorrência de série DIFERIDA (backlog pós-Epic-4). **GAP-4.6 resolvido** | FR34 | `@dev` | `@architect` | **DONE** — PR #55 squash-merged `6b429560` (03/06); hotfix middleware (`/api/push/dispatch` 307→) PR #56 `017a032c` em produção. Gate de saída Aria PASS Confidence High (4 DEV-DECISIONS ratificadas; D-KV-HASH/D-RECON-MOUNT code-discovered superiores à entrada; Major CR = falso positivo single-user). `CRON_SECRET` provisionado + scheduler externo cron-job.org LIVE ao minuto (test run 200). AC8 (NFR23) validado por evidência acumulada (recepção Chrome+Edge do smoke 4.7 + disparo agendado 401/200 da 4.8), fecho autorizado pelo Eurico sem re-execução manual. CR Iter 2 0 CRITICAL, Vitest 1329/1329, zero waivers. Backlog AC5 (recorrência de lembretes server-side) `@po`/`@pm` |
| 4.9 | Service Worker push handler | SW handler de push notifications: mostra notificação + botões "marcar feito"/"snooze 10min" accionáveis sem abrir a app (FR36). **GAP: SW mínimo no Epic 4 vs SW completo no Epic 8 (story 8.3) — ver §7** | FR36 | `@dev` | `@architect` | **DONE** — PR #58 squash-merged `64a41445` (06/06). SW handler `push` real (display visível) + `notificationclick` com botões "marcar feito"/"snooze 10min" → `/api/push/action` (cookie same-origin, D-ACTION-AUTH-COOKIE); extensão GET `/api/push/schedule` filtra snoozes (`snoozedAt`, D-SNOOZE-CONTRACT) + reconciliação `reconcile-snooze.ts` on-mount. Architect Gate Aria 4 iter (Iter 4 PASS; Iter 3 reabriu 4 Major CR de semântica snooze → D-SNOOZE-CONTRACT, RF1-RF7); CR 0 CRITICAL, CI 100% verde rollup CLEAN, Vitest 1383/1383, zero waivers. **AC13** (verificação manual Chrome+Edge) fica para produção pós-merge, autorizada pelo Eurico (preview sem env vars VAPID). |
| 4.10 | Tools cérebro | Registar 9 tools no Tool Registry (nomes ASCII validados — ver §4 e nota abaixo): `criar_habito`, `registar_habito_concluido`, `consultar_evolucao_habito`, `criar_meta`, `actualizar_meta`, `consultar_metas`, `criar_lembrete`, `listar_lembretes`, `cancelar_lembrete` | FR28, FR41, FR38 | `@dev` | `@architect` | **DONE** — PR #53 merged `09d52b24` (01/06). **D-DOMAIN ratificada (Aria) Opção A:** as 9 tools usam `domain:'habits'` (classifier já agrupa as 3 áreas; precedente A10 `projects.ts:'tasks'`) — enum `ToolDomain` NÃO estendido. 3 módulos Edge-safe (`habits.ts`/`goals.ts`/`reminders.ts`, sem import client/repos, `ctx.db`). Reverse por snapshot. Recorrência inline (RRule pura). Architect Gate PASS (D1-D4 + Edge-safety + reverse verificados no código + 119 testes de tools reproduzidos), CR 0 CRITICAL, Vitest 1274/1274, zero waivers. Desbloqueia o cérebro a operar hábitos/metas/lembretes por linguagem natural (AC4 epic) |
...
**Epic 4 FECHADO — 10/10 stories Done (4.1, 4.2, 4.3, 4.4, 4.5, 4.6, 4.7, 4.8, 4.9, 4.10).** As Stories 4.1 (schema/DAL, PR #41 merge `aa74ce56`), 4.2 (CRUD hábitos, PR #42), 4.3 (heatmap calendário), 4.4 (métricas por hábito, PR #50 merge `192b488c`), 4.5 (CRUD metas, PR #52 merge `a7078291`), 4.6 (CRUD lembretes, PR #51 merge `d13a6067`), 4.7 (setup Web Push, PR #54 merge `25d1c780`), 4.8 (agendamento push server-side, PR #55 merge `6b429560` + hotfix PR #56 `017a032c`), 4.9 (SW push handler, PR #58 merge `64a41445`) e 4.10 (tools cérebro, PR #53 merge `09d52b24`) estão DONE em `main`. **Web Push completo end-to-end:** subscrição (4.7) → agendamento server-side + dispatch ao minuto (4.8) → SW handler com display visível + botões accionáveis (4.9). **Próximo passo:** Eurico faz a verificação manual da AC13 da 4.9 (display + botões em Chrome+Edge) em produção pós-merge (autorizada — preview do PR sem env vars VAPID, produção tem) + `@po *retrospective epic-4`. As decisões arquitecturais da 4.1 estão fixadas: `goal_milestones` EMBEBIDO (sem `version(5)`); convenção de delete-cascata = *cascade nos filhos sem vida própria (composição); set null nas referências a entidades com vida própria (associação)*; hard-delete; corolário D6 (`Task.projectId` → set null) ratificado como referência para a story técnica pós-Epic-4.
