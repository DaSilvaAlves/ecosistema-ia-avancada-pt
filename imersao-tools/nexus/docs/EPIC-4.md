...
> **Estado:** EM CURSO — 7/10 stories Done (4.1 schema/DAL, 4.2 CRUD hábitos, 4.3 heatmap calendário, 4.4 métricas por hábito, 4.5 CRUD metas, 4.6 CRUD lembretes, 4.10 tools cérebro). Restam só as 3 de Web Push: 4.7 (setup — maior risco arquitectural), 4.8 (agendamento), 4.9 (SW handler). 4.7 aguarda VAPID env vars no Vercel.
...
**Epic 4 EM CURSO — 7/10 stories Done (4.1, 4.2, 4.3, 4.4, 4.5, 4.6, 4.10).** As Stories 4.1 (schema/DAL, PR #41 merge `aa74ce56`), 4.2 (CRUD hábitos, PR #42), 4.3 (heatmap calendário), 4.4 (métricas por hábito, PR #50 merge `192b488c`), 4.5 (CRUD metas, PR #52 merge `a7078291`), 4.6 (CRUD lembretes, PR #51 merge `d13a6067`) e 4.10 (tools cérebro, PR #53 merge `09d52b24`) estão DONE em `main`. Restam 3 stories — todas de Web Push: 4.7 (setup — maior risco arquitectural), 4.8 (agendamento push) e 4.9 (SW push handler). **Próxima: 4.7** (aguarda VAPID env vars no Vercel — destranca toda a cadeia 4.7→4.8→4.9). As decisões arquitecturais da 4.1 estão fixadas: `goal_milestones` EMBEBIDO (sem `version(5)`); convenção de delete-cascata = *cascade nos filhos sem vida própria (composição); set null nas referências a entidades com vida própria (associação)*; hard-delete; corolário D6 (`Task.projectId` → set null) ratificado como referência para a story técnica pós-Epic-4.
