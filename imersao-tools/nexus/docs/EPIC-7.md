...
|---|-------|--------------------------------------|-----|-------------------|------------------------|--------|
| 7.1 | Componente VoiceMode | Botão microfone no chat principal + indicador visual + stream. **Múltiplos estados de render (inactivo/a-ouvir/a-processar/erro/não-suportado) → teste de componente (`react-component-test-criteria.md`)** | FR77 | `@ux-design-expert` | `@dev` | Não iniciado |
