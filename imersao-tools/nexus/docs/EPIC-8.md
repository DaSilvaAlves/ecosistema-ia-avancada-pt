...
> **Estado:** **FECHADO — 6/6 stories Done** (8.6 em âmbito revisto: runbook + capacidade dual-provider entregues; cutover LIVE deferido on-demand — correct-course 01/07/2026, `REC-8.6-CUTOVER-DEFERIDO`; ver §10). Story 8.1 (S1, fundação) **Done** — merged em `main` via **PR #95** (squash, `dec0b203`) a 25/06/2026, Architect Gate de saída PASS (Confiança Alta), CR `--base main` 1 iteração (1 Major guard de nome de tool + 2 Minor resolvidos em `40dc1fdb`, re-review APPROVED), waiver 0%. **A 8.1 desbloqueia a 8.2 (`OpenAIExecutor`) e a 8.3 (`OpenAIClassifier`), agora paralelizáveis** (ADR-10 §8 caminho crítico `S1 → S2/S3`). **8.2 (`OpenAIExecutor`) Done** (PR #96, squash `29ba4046` em `main`, 26/06/2026; Architect Gate de saída PASS, CR `--base main` 0 findings Major, waiver 0%); **8.3 (`OpenAIClassifier`) Done** (PR #97, squash `fc74ea89` em `main`, 27/06/2026; Architect Gate de saída PASS Confiança Alta, CR server-side `--base main` 2 Minor não-bloqueadores registados como dívida REC-8.3-CR-1/2, 0 Major, waiver 0%); **8.4 (proxy `/api/openai/proxy` Edge + `OpenAIInferenceTransport` + `sse-lines.ts`, caminho QUENTE) Done** (PR #98, squash `839d0828` em `main`, 27/06/2026; Architect Gate de saída PASS Confiança Alta, CR server-side `--base main` Iter 2 — 4 findings aplicados pelo `@dev` + 1 Major rate-limit ratificado como dívida partilhada REC-8.4-CR-1 com thread CR respondida e resolvida, 0 Major acionável por resolver, waiver 0%); **8.5 (MSW `handlers/openai.ts` non-streaming + `proxy-fetch-openai.ts` + suite de parity cross-provider) Done** (PR #99, squash `e082edf4` em `main`, 29/06/2026; executor `@dev` → gate `@qa` PASS, CR server-side `--base main` Iter 2 — 2 Major + 4 Minor da Iter 1 aplicados pelo `@dev` no commit `07516ea3`, re-review APPROVED, 0 Major por resolver, waiver 0%); **8.6 (runbook cutover/rollback + capacidade dual-provider validada) Done** (PR #100, squash `eb5d18a5` em `main`, 01/07/2026; gate PO GO 9/10 + gate QA PASS, CR `--base main` Iter 1 APPROVED, waiver 0%; **âmbito revisto — cutover LIVE deferido on-demand**, ver §10). **Este é um epic não previsto no roadmap original do PRD (§9), criado em resposta a um gatilho de produção:** a 25/06/2026 a Anthropic Messages API passou a devolver `400 credit balance too low` e a produção do Nexus v2 ficou **sem cérebro** (todo o pipeline classifier→executor→tools depende de `api.anthropic.com`). O Eurico decidiu **não recarregar créditos Anthropic** e migrar a camada de inferência para a OpenAI (decisão vinculativa — ADR-10 §1.2). Por bloquear produção, esta migração **tem prioridade sobre o Hardening**, que desliza para **Epic 9** (ver nota de numeração abaixo).
...
| 8.3 | **`OpenAIClassifier` (server, JSON)** (S3) | `chat.completions` não-streaming + `response_format:{type:'json_object'}`; `stripJsonMarkdownFences` defensivo; devolve `ClassificationResult` validado por Zod; usage mapeada | **`@architect`** (parser AI — a mais leve das três) | **Done (PR #97, `fc74ea89`)** (`stories/completed/8.3.story.md`) |
| 8.4 | **Proxy OpenAI + transport client** (S4) | `/api/openai/proxy` (Edge, auth `getSession`, rate-limit KV, upstream `api.openai.com/v1/chat/completions`, `Authorization: Bearer`); `OpenAIInferenceTransport` (body+SSE OpenAI); `client-executor` selecciona por `NEXT_PUBLIC_LLM_PROVIDER`; extrair `iterateSseData`→`sse-lines.ts` | **`@architect`** (endpoint Edge + input externo — **CR `--base main` obrigatório**) | **Done (PR #98, `839d0828`)** (`stories/completed/8.4.story.md`) |
| 8.5 | **MSW + parity tests** (S5) | `tests/mocks/handlers/openai.ts` (SSE fiel: args fragmentados em ≥2 deltas, chunk de usage, `[DONE]`, `finish_reason`); variante OpenAI de `proxy-fetch.ts`; suite de parity cross-provider nos 6 cenários canónicos | **`@dev`** autora (decisão Eurico) → gate **`@qa`** (`separation-of-roles.md`) | **Done (PR #99, squash `e082edf4`)** (`stories/completed/8.5.story.md`) |
| 8.6 | **Cutover + runbook** (S6) | Runbook de cutover/rollback (flip `anthropic`↔`openai`) documentado e testável; capacidade dual-provider validada. **Cutover LIVE OpenAI deferido on-demand** (correct-course 01/07 — ver §6 AC3 + `REC-8.6-CUTOVER-DEFERIDO`) | **`@qa`** + manual (deploy por `@devops`) | **Done (âmbito revisto) (PR #100, `eb5d18a5`)** (`stories/completed/8.6.story.md`) |

**Caminho crítico (ADR-10 §8):** `S1 → S2/S3 (paralelizáveis após S1) → S4 → S5 → S6`. A 8.5 depende de 8.2 + 8.4 (precisa dos dois caminhos para testar parity). A 8.6 é a **única story com efeito em produção** e só arranca com a parity (8.5) verde + key validada. A 8.1 (**Done** — PR #95, `dec0b203`) desbloqueou todas. A **8.2 (`OpenAIExecutor`) está Done** (PR #96, `29ba4046`) e a **8.3 (`OpenAIClassifier`) está Done** (PR #97, `fc74ea89`) — ambas no mesmo ficheiro `openai.ts`, a 8.3 reutilizou os helpers da 8.2 (`isOpenAITestEnv`/`buildOpenAIClientOptions`/sentinela). A **8.4 (proxy Edge + transport client) está Done** (PR #98, `839d0828`) — o caminho client OpenAI (quente) está completo. A **8.5 (MSW `handlers/openai.ts` non-streaming + `proxy-fetch-openai.ts` + parity cross-provider) está Done** (PR #99, squash `e082edf4`); a parity está verde nos 6 cenários canónicos. O próximo e último trabalho é a **8.6** (cutover de produção), agora desbloqueado.
...
| AC3 | **[Revisto — correct-course 01/07/2026]** Runbook de cutover/rollback documentado e testável (flip `anthropic`↔`openai`) — **CUMPRIDO** (`docs/runbooks/cutover-openai-rollback.md`, PR #100, gate QA PASS). O **cutover LIVE para OpenAI em produção é DEFERIDO on-demand**: a premissa que o forçava (saldo Anthropic esgotado, §1.1) deixou de se verificar — a produção responde via Anthropic, verificado por smoke test 01/07/2026. Trace item pendente `REC-8.6-CUTOVER-DEFERIDO`. *(Formulação original: "A produção responde via OpenAI após o cutover" — não cumprida por decisão consciente do Eurico, não por falha.)* | ADR-10 §8 row S6 | 8.6 (cutover) |
...
> **Estado: Epic 8 FECHADO — 6/6 stories Done (âmbito da 8.6 revisto — correct-course 01/07/2026).** Stories 8.1 (fundação, PR #95, `dec0b203`), 8.2 (`OpenAIExecutor`, PR #96, `29ba4046`), 8.3 (`OpenAIClassifier`, PR #97, `fc74ea89`), 8.4 (proxy `/api/openai/proxy` Edge + transport client, PR #98, `839d0828`), 8.5 (MSW non-streaming + `proxy-fetch-openai.ts` + parity cross-provider, PR #99, `e082edf4`) e 8.6 (runbook cutover/rollback + capacidade dual-provider validada, PR #100, `eb5d18a5`) **Done**.
>
> **Critério de fecho (revisto):** a promessa de valor do Epic 8 é a **capacidade dual-provider** (ADR-10 §1.2.3 — "o critério é correcção, não uptime"), **entregue e verde**: 6/6 stories Done, parity cross-provider verde nos 6 cenários (8.5), baseline 2536 PASS / 0 FAIL real, runbook validado (8.6). O **cutover LIVE para OpenAI em produção foi DEFERIDO** (item `REC-8.6-CUTOVER-DEFERIDO`) por decisão do Eurico (01/07/2026): a produção responde via **Anthropic** (a premissa "saldo esgotado" que exigia o cutover deixou de se verificar). O cutover passa a alavanca **on-demand** executável pelo runbook em minutos se/quando o Anthropic esgotar. Isto **não bloqueia o Epic 9** — o cérebro está vivo em produção e a capacidade de comutar está pronta. O roadmap segue para o **Epic 9 (Hardening + Deploy + PWA)**.
>
> *(Formulação original do critério: "6/6 stories Done com a parity (8.5) verde e o cutover (8.6) validado em produção pelo Eurico" — o cutover LIVE não foi executado; ver correct-course na Story 8.6 e `REC-8.6-CUTOVER-DEFERIDO`.)*
...
- **8.4** (proxy `/api/openai/proxy` Edge + `OpenAIInferenceTransport` + selecção client + `sse-lines.ts`) → **Done (PR #98, `839d0828`)**. Architect Gate PASS + CR `--base main` Iter 2 (4 fixes aplicados; 1 Major rate-limit = dívida partilhada REC-8.4-CR-1).
...
### Item pendente pós-fecho — cutover LIVE deferido (correct-course 01/07/2026)

| ID | Descrição | Dono | Gatilho | Acção |
|----|-----------|------|---------|-------|
| `REC-8.6-CUTOVER-DEFERIDO` | Cutover LIVE da inferência de produção **Anthropic → OpenAI** (flip `LLM_PROVIDER=openai` + `NEXT_PUBLIC_LLM_PROVIDER=openai` + provisão da `OPENAI_API_KEY` na Vercel + redeploy + smoke test). Corresponde aos AC1/AC2/AC3(letra)/AC6 da Story 8.6, **deferidos** por decisão do Eurico (01/07/2026) — a premissa que os forçava (saldo Anthropic esgotado, §1.1) deixou de se verificar; a produção responde via Anthropic. | **Eurico + `@devops`** | "se/quando o Anthropic esgotar (saldo/quota) **ou** por decisão de negócio" | Seguir o procedimento de cutover em `docs/runbooks/cutover-openai-rollback.md` (§3); rollback pelo mesmo runbook (§4). Não é uma story nova — é um procedimento operacional já documentado e validado (AC4). On-demand, sem SLA. |

> Este item **não bloqueia o Epic 9**: o cérebro está vivo em produção (via Anthropic) e a capacidade de comutar de provider está pronta e testada (parity verde + runbook). O cutover é uma alavanca on-demand, não uma dívida por resolver antes de avançar.
