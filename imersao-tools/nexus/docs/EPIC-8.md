...
> **Estado: Epic 8 EM CURSO — 5/6 stories Done.** Stories 8.1 (fundação, PR #95, `dec0b203`), 8.2 (`OpenAIExecutor`, PR #96, `29ba4046`), 8.3 (`OpenAIClassifier`, PR #97, `fc74ea89`), 8.4 (proxy `/api/openai/proxy` Edge + transport client, PR #98, `839d0828`) e 8.5 (MSW non-streaming + `proxy-fetch-openai.ts` + parity cross-provider, PR #99, `e082edf4`) **Done**; resta só: 8.6 (cutover de produção). Critério de fecho: **6/6 stories Done** com a parity (8.5) verde e o cutover (8.6) validado em produção pelo Eurico. Com o Epic 8 fechado, o cérebro do Nexus volta a responder em produção (via OpenAI), a Anthropic fica como fallback atrás da flag, e o roadmap segue para o **Epic 9 (Hardening + Deploy + PWA)**.
