...
> **Estado:** **EM CURSO — 3/17 stories Done.** Sucessor natural do Epic 5 (FECHADO 13/13, waiver rate 0%) na ordem PRD §9 (`4 → 5 → 6`). É o **primeiro epic de integrações externas autenticadas** do Nexus: OAuth Google (Calendar + Gmail) + canal Telegram bot. Story 6.1 (OAuth flow Google, Calendar scope, FR58) FECHADA 17/06/2026 via PR #75 (squash `e7e4994d`) — fundação OAuth reutilizada pelas 6.2-6.7. Story 6.2 (Refresh token storage Vercel KV, encriptação at-rest + refresh flow + revogação, FR60, GAP-6.2) FECHADA 17/06/2026 via PR #76 (squash `1621e8a6`), waiver 0 — `getValidAccessToken()` é o único ponto de entrada de access token para 6.3/6.4/6.6/6.7. Story 6.3 (Sync calendário pull, eventos Google → Nexus via syncToken incremental, FR59) FECHADA 17/06/2026 via PR #77 (squash `e149fc76`), waiver 0 — helper puro `lib/google/calendar.ts` (96,62% cobertura) + cursor KV dedicado + version(6) Dexie `calendarEvents`; CodeRabbit `--base main` 0 findings na Iter 1 (após fix D-6.3-POSITIVE-EPOCH). Próximo passo: `@sm *draft 6.4` (sync calendário push — Nexus → Google, completa o 2-way; gate `@architect` por estado distribuído).
...
| 6.4 | Sync calendário push | Eventos Nexus → Google. Completa o 2-way. **Estado distribuído (criado/sincronizado/conflito) → `internal-state-contract-gate.md` aplica-se (ver §8)** | FR59 | `@dev` | `@architect` | Draft |
...
**Epic 6 EM CURSO — 3/17 stories Done.** Sucessor natural do Epic 5 (FECHADO 13/13, waiver 0%) na ordem PRD §9 (`4 → 5 → 6`); o Epic 6 **bloqueia o Epic 7** (Voice + OCR integra-se com o canal Telegram). Os três sub-módulos (Calendar 6.1-6.6, Gmail 6.7-6.10, Telegram 6.11-6.17) assentam na fundação OAuth (6.1+6.2 — DONE). A 6.3 (sync pull) está fechada; a 6.4 (sync push) completa o 2-way de calendário. O quality gate específico do PRD §10 ("revisão segurança tokens OAuth") concentra-se nas stories de OAuth/token/webhook, com Architect Gate de Entrada recomendado. Os GAPs restantes (`[GAP-6.3]`, `[GAP-6.4]`, `[GAP-6.5]`, `[GAP-6.6]`) ficam marcados para o `@architect` resolver no draft das stories respectivas.

**Story 6.4 (sync calendário push — Nexus → Google) é a próxima a arrancar via `/sdc`** — completa o 2-way (FR59) sobre a fundação 6.1+6.2+6.3. É território de **estado distribuído** (evento criado localmente → push para Google → conflito quando o Google alterou o mesmo evento): aplica-se `internal-state-contract-gate.md` (lição A1 Epic 4) com **gate `@architect`** e análise de ciclo de vida no gate. Reutiliza a reconciliação por `googleId` e o cursor `nexus:google:calendar:syncToken` da 6.3 para evitar loop de sync (Risco R6).

### Próximas acções na sequência

1. **`@sm` (River)** — `*draft 6.4` (sync calendário push — Nexus → Google) — completa o 2-way de calendário.
2. **`@architect` (Aria)** — **Architect Gate de Entrada** da 6.4: análise de ciclo de vida do estado distribuído (criado/sincronizado/conflito) + idempotência por `googleId`/etag para não criar loop ou duplicados (Risco R6, `internal-state-contract-gate.md`).
3. **`@po` (Pax)** — `*validate-story-draft 6.4` (10-point checklist).
4. **`@dev` (Dex)** — `*develop 6.4` com gate `@architect` (Aria).
5. **Em paralelo (opcional):** `@sm *draft 6.5` (cron sync delta — depende de 6.3/6.4 e serve também o trigger Gmail), `*draft 6.6` (tools cérebro calendário — lê os eventos sincronizados pela 6.3), ou `*draft 6.11` (Telegram setup — sub-módulo independente, pode arrancar em paralelo com o Google). Architect Gate de Entrada recomendado para 6.11/6.12 (webhook + decisão SDK Telegram + 2 criticals `npm audit`, `[GAP-6.4]`).
