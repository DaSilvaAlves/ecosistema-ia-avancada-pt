...
> **Estado:** **EM CURSO — 2/17 stories Done.** Sucessor natural do Epic 5 (FECHADO 13/13, waiver rate 0%) na ordem PRD §9 (`4 → 5 → 6`). É o **primeiro epic de integrações externas autenticadas** do Nexus: OAuth Google (Calendar + Gmail) + canal Telegram bot. Story 6.1 (OAuth flow Google, Calendar scope, FR58) FECHADA 17/06/2026 via PR #75 (squash `e7e4994d`) — fundação OAuth reutilizada pelas 6.2-6.7. Story 6.2 (Refresh token storage Vercel KV, encriptação at-rest + refresh flow + revogação, FR60, GAP-6.2) FECHADA 17/06/2026 via PR #76 (squash `1621e8a6`), waiver 0 — `getValidAccessToken()` é o único ponto de entrada de access token para 6.3/6.4/6.6/6.7. Próximo passo: `@sm *draft 6.3` (sync calendário pull, FR59).
