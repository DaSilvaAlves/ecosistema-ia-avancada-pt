...
> **Estado:** **EM CURSO — 1/17 stories Done.** Sucessor natural do Epic 5 (FECHADO 13/13, waiver rate 0%) na ordem PRD §9 (`4 → 5 → 6`). É o **primeiro epic de integrações externas autenticadas** do Nexus: OAuth Google (Calendar + Gmail) + canal Telegram bot. Story 6.1 (OAuth flow Google, Calendar scope, FR58) FECHADA 17/06/2026 via PR #75 (squash `e7e4994d`) — fundação OAuth reutilizada pelas 6.2-6.7. Próximo passo: `@sm *draft 6.2` (refresh token storage, GAP-6.2).
