...
> **Estado:** **PLANEADO (Draft) — 0/17 stories.** Sucessor natural do Epic 5 (FECHADO 13/13, waiver rate 0%) na ordem PRD §9 (`4 → 5 → 6`). É o **primeiro epic de integrações externas autenticadas** do Nexus: OAuth Google (Calendar + Gmail) + canal Telegram bot. Próximo passo após criação: `@po *retrospective epic-5` (ainda pendente do Epic 5) e depois `@sm *draft 6.1`.
...
|---|-------|--------------------------------------|-----|-------------------|------------------------|--------|
| 6.1 | OAuth flow Google (Calendar scope) | UI definições para iniciar OAuth (scope `calendar`) + callback handler `/api/google/oauth/*` (Node, `googleapis`). **GAP segurança — ver §7 GAP-6.1** | FR58 | `@dev` | `@architect` | Draft |
| 6.2 | Refresh token storage Vercel KV | Armazenar refresh token server-side em Vercel KV (`nexus:google:tokens`, arch §6). Refresh flow + revogação. **GAP segurança — ver §7 GAP-6.2** | FR60 | `@dev` | `@architect` | Draft |
| 6.3 | Sync calendário pull | Eventos Google → Nexus (sync incremental via syncToken Google). Helper puro de reconciliação em `lib/google/calendar.ts` | FR59 | `@dev` | `@qa` | Draft |
...
**Epic 6 PLANEADO — 0/17 stories.** Sucessor natural do Epic 5 (FECHADO 13/13, waiver 0%) na ordem PRD §9 (`4 → 5 → 6`); o Epic 6 **bloqueia o Epic 7** (Voice + OCR integra-se com o canal Telegram). Os três sub-módulos (Calendar 6.1-6.6, Gmail 6.7-6.10, Telegram 6.11-6.17) assentam na fundação OAuth (6.1+6.2). O quality gate específico do PRD §10 ("revisão segurança tokens OAuth") concentra-se nas stories de OAuth/token/webhook, com Architect Gate de Entrada recomendado. Os 6 GAPs (`[GAP-6.1]`-`[GAP-6.6]`) ficam marcados para o `@architect` resolver no draft das stories respectivas.

**Story 6.1 (OAuth flow Google — Calendar scope) é a primeira a arrancar via `/sdc`** — é a fundação de toda a integração Google (Calendar + Gmail partilham o cliente OAuth) e o ponto de maior risco de segurança. Recomenda-se Architect Gate de Entrada antes do `@dev` (resolver GAP-6.1 + GAP-6.2 cedo).

### Próximas acções na sequência

1. **`@po` (Pax)** — `*retrospective epic-5` (pendente; pode gerar acções A* a aplicar ao Epic 6 antes de arrancar).
2. **`@aiox-master`/`@architect`** — confirmar que a regra `internal-state-contract-gate.md` existe (roadmap P2.4, "antes do Epic 6").
3. **Eurico + `@devops`** — provisionar credenciais Google OAuth (Client ID/Secret/Redirect URI) e token BotFather em Vercel env (arch §9.2) — pré-requisito da 6.1/6.11.
4. **`@architect` (Aria)** — **Architect Gate de Entrada** da 6.1+6.2 (OAuth + token storage) para resolver `[GAP-6.1]` e `[GAP-6.2]` antes da implementação; idem 6.11+6.12 (webhook + decisão SDK Telegram + 2 criticals `npm audit`, `[GAP-6.4]`).
5. **`@sm` (River)** — `*draft 6.1` (OAuth flow Google Calendar scope) — fundação de Calendar + Gmail.
6. **`@po` (Pax)** — `*validate-story-draft 6.1` (10-point checklist).
7. **`@dev` (Dex)** — `*develop 6.1` com gate `@architect` (Aria).
