...
**Epic 6 EM CURSO — 10/17 stories Done.** Sucessor natural do Epic 5 (FECHADO 13/13, waiver 0%) na ordem PRD §9 (`4 → 5 → 6`); o Epic 6 **bloqueia o Epic 7** (Voice + OCR integra-se com o canal Telegram). Os três sub-módulos (Calendar 6.1-6.6, Gmail 6.7-6.10, Telegram 6.11-6.17) assentam na fundação OAuth (6.1+6.2 — DONE). **O sub-módulo Calendar está COMPLETO (6/6: 6.1-6.6).** **O sub-módulo Gmail está COMPLETO (4/4: 6.7 OAuth scope `gmail.modify`, 6.8 classifier 4 buckets, 6.9 vista no dashboard, 6.10 tools cérebro Gmail).** Resta apenas o sub-módulo Telegram (6.11-6.17), a 0/7. O quality gate específico do PRD §10 ("revisão segurança tokens OAuth") concentra-se nas stories de OAuth/token/webhook, com Architect Gate de Entrada recomendado. Os GAPs do sub-módulo Telegram ficam marcados para o `@architect` resolver no draft das stories respectivas.

**Próxima candidata (sub-módulo Telegram — único restante):**
- **6.11 (Telegram bot setup)** — sub-módulo **totalmente independente** do Google; abre o sub-módulo Telegram. Token BotFather em Vercel env; webhook `/api/telegram/webhook` registado via `setWebhook` com `secret_token` (arch §9.5). Gate `@architect` **obrigatório** (webhook público + decisão SDK Telegram `[GAP-6.4]` + 2 criticals `npm audit` na cadeia `request` via `node-telegram-bot-api` — manter, migrar para `grammY`/`telegraf`, ou usar a Bot API por `fetch` directo; Edge favorece `fetch`). É o próximo passo natural (Gmail e Calendar fechados).
- **6.12 (webhook handler)** — depende de 6.11; Edge handler que valida `x-telegram-bot-api-secret-token`, faz parse de texto/voz/foto e roteia (fan-out <5s). Gate `@architect`.

### Próximas acções na sequência

1. **`@sm` (River)** — `*draft 6.11` (Telegram bot setup — abre o sub-módulo Telegram, único restante; Calendar e Gmail COMPLETOS).
2. **`@architect` (Aria)** — **Architect Gate de Entrada** da 6.11 **obrigatório** (webhook público + decisão SDK Telegram + 2 criticals `npm audit`, `[GAP-6.4]`).
