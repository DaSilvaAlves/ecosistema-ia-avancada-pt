...
**Epic 6 EM CURSO — 9/17 stories Done.** Sucessor natural do Epic 5 (FECHADO 13/13, waiver 0%) na ordem PRD §9 (`4 → 5 → 6`); o Epic 6 **bloqueia o Epic 7** (Voice + OCR integra-se com o canal Telegram). Os três sub-módulos (Calendar 6.1-6.6, Gmail 6.7-6.10, Telegram 6.11-6.17) assentam na fundação OAuth (6.1+6.2 — DONE). **O sub-módulo Calendar está COMPLETO (6/6: 6.1-6.6).** **O sub-módulo Gmail está a 3/4: 6.7 (OAuth scope `gmail.modify`), 6.8 (classifier 4 buckets) e 6.9 (vista no dashboard) fechadas — falta só 6.10 (tools cérebro Gmail).** O sub-módulo Telegram (6.11-6.17) está 0/7. O quality gate específico do PRD §10 ("revisão segurança tokens OAuth") concentra-se nas stories de OAuth/token/webhook, com Architect Gate de Entrada recomendado. Os GAPs do sub-módulo Telegram ficam marcados para o `@architect` resolver no draft das stories respectivas.

**Próximas candidatas (sequência não rígida — `@sm`/`@po` confirmam paralelizabilidade):**
- **6.10 (tools cérebro Gmail)** — `listar_emails_importantes`/`criar_draft_gmail`/`arquivar_email` no Tool Registry (domínio `gmail`). `criar_draft_gmail` com `requiresPreview: true` (acção externa). Consome o mesmo contrato KV que a vista 6.9 lê (`nexus:cache:gmail:classify:<msgId>`) — fecha o sub-módulo Gmail (4/4). Nomes ASCII já validados (nota §5). Gate `@architect`. **É o próximo passo natural** (completa o Gmail antes de abrir o Telegram).
- **6.11 (Telegram bot setup)** — sub-módulo **totalmente independente** do Google; pode arrancar em paralelo. Gate `@architect` (webhook + decisão SDK Telegram `[GAP-6.4]` + 2 criticals `npm audit`).
