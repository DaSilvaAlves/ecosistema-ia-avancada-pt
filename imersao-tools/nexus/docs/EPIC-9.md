...
> **Estado: Epic 9 EM CURSO — 2/11 unidades de âmbito Done: 9.11 DONE (01/07/2026) + unidade 9.1 COMPLETA (9.1a PR #102 `b21bb0c2` + 9.1b PR #103 `88f18b43`, ambas 03/07/2026).** A unidade de âmbito 9.1 (cobertura ≥60% packages core, NFR17) está **fechada** — os três packages core (cérebro, tarefas, finanças) passam agora pelo gate de cobertura. Restam 9.2-9.10 (9 unidades). Último epic do roadmap PRD §9. **Não bloqueia nenhum epic** (coluna "Bloqueia" vazia). Critério de fecho: **11/11 stories Done** — a unidade 9.1 lê-se como "9.1a **e** 9.1b Done" (ver nota de numeração em §5), **já cumprida** — com CI bloqueante activo (9.8), PWA instalável verificada (AC3), backup/restore round-trip validado (AC4/AC5), deploy contínuo automatizado (9.10, NFR19) e o full-suite determinístico (9.11, já feito). Com o Epic 9 fechado, o Nexus v2 está **production-ready** e o roadmap MVP (PRD §9) fica completo. Próximo passo: **9.2 (E2E Playwright caminho crítico)** — independente, gate `@qa`, sem dependência de estado de produção → `@sm *draft 9.2`. As stories de deploy/CI (9.8/9.10) continuam a exigir a resolução das incógnitas de estado de produção (a)/(b) abaixo (9.10 bloqueada por `production-state-verification-gate.md`); 9.8 desbloqueada pela 9.11.
